  ...
  return (
    <div className="rounded-xl border border-gray-800 bg-gray-900/50 p-6 hover:border-gray-700 transition-colors">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="space-y-1"><div className="flex items-center gap-2 text-gray-400"><Activity className="h-4 w-4" /><span className="text-xs">Renewable</span></div><p className="text-lg font-semibold text-gray-100">{formatPercentage(cluster.renewablePercentage)}</p><p className="text-xs text-gray-500">{cluster.renewablePercentage > 70 ? 'Excellent' : 'Good'}</p></div>
      </div>
      <div className="mt-4">
        <div className="flex justify-between text-xs mb-1"><span className="text-gray-400">Capacity</span><span className="text-gray-300">{formatPercentage(cluster.utilization)}</span></div>
        <div className="h-2 bg-gray-800 rounded-full overflow-hidden">
          <div className={cn("h-full rounded-full transition-all", cluster.utilization > 90 ? "bg-red-500" : cluster.utilization > 75 ? "bg-yellow-500" : "bg-green-500")} style={{ width: `${cluster.utilization}%` }} />
        </div>
      </div>
    </div>
  );
